...
- [백업 훅](#백업-훅)
- [마스터 암호화 키](#마스터-암호화-키)
- [관리자 비밀번호를 잊었을 때](#관리자-비밀번호를-잊었을-때)
...
부트스트랩 비밀번호는 **첫 실행 때 터미널에 한 번만** 찍힌다. 로그 파일에는 남기지 않는다 —
구조화 로그 수집기에 비밀번호가 파싱되어 저장되는 것을 피하려는 것이다. 그 터미널을 닫았고
다른 관리자 계정도 없으면 들어갈 길이 없다: 저장된 것은 argon2id 해시라 되돌릴 수 없다.

그때는 **서버가 도는 기계에서** 다음을 실행한다.

```bash
dbstudio reset-password                        # superadmin
dbstudio reset-password -data /srv/dbstudio/data
dbstudio reset-password -data ./data dev1      # 다른 계정
```

새 랜덤 비밀번호가 터미널에 한 번 찍히고, **첫 로그인에서 변경이 요구된다**. 그 계정의 세션은
모두 끊긴다(예전 비밀번호로 받은 쿠키가 남으면 바꾼 의미가 없다).

- **서버를 멈추지 않아도 된다.** 비밀번호는 어디에도 캐시되지 않으므로 바로 듣는다. 대신
  그 계정으로 열려 있던 브라우저 창은 로그아웃된다
- **데이터 디렉터리와 마스터 키가 있어야 한다.** `-data`(또는 `DBSTUDIO_DATA`)가 가리키는 곳에
  `dbstudio.db` 가 없으면 실행을 거절한다 — 없는 것을 열면 빈 DB 를 새로 만들고 "그런 계정이
  없습니다"라고 답하게 되는데, 진짜 원인은 디렉터리를 잘못 짚은 것이다.
  마스터 키가 없어 새로 만들어지는 경우도 같은 이유로 거절한다
- **클러스터에서는 마스터 노드에서** 실행한다. 리플리카에서 실행하면 거절한다 — 사용자는
  마스터가 들고 있어서, 리플리카에서 바꿔도 다음 동기화에 조용히 되돌려진다
- 실행 사실은 **감사 기록**에 남는다(행위자 `cli`). 이 명령은 화면의 권한 검사를 거치지 않으므로,
  누가 언제 했는지가 남는 곳은 그 줄뿐이다

이 명령을 쓸 수 있는 사람은 데이터 디렉터리와 `master.key` 를 읽을 수 있는 사람이다. 그 둘을 가진
사람은 이미 저장된 모든 DB 자격증명을 풀 수 있으므로, 이 명령이 권한의 문을 낮추지는 않는다.
반대로 말하면 **그 파일들의 접근 권한이 곧 이 앱의 최종 권한**이다.

## 배포본 판정 — 이 노드가 어느 커밋인가

장애 조사에서 "이 증상이 코드 탓인가, 배포본이 낡았나"를 먼저 갈라야 할 때가 있다.
담당 노드 중계는 커밋 단위로 기능이 늘었으므로, **어느 화면이 되는지로 배포 시점을
역산할 수 있다.**

| 화면 | 이 기능이 들어간 커밋 | 그 이전 배포본의 증상 |
|---|---|---|
| 구조 화면 (`/structure`) | `f1bc665` (2026-09-17 17:16 KST = 08:16 UTC) | 담당 노드로 넘어가지 못하고 **마스터가 실행** → 마스터가 그 DB에 닿지 못하면 1045 |
| 지표·버전 캡처·백업·복구 | `425b032` / `6b70235` / `bd52c5a` (2026-09-20) | 마스터가 직접 접속 → 1045 |

**판정 방법**: 담당 노드가 지정된 DB에서 구조 화면을 열어 본다.

- **성공** → `f1bc665` 이후 배포본이다(구조 화면이 담당 노드를 지난다)
- **실패(1045)** → `f1bc665` 이전 배포본이거나, 담당 노드의 주소를 알 수 없는 상태다

**응답 헤더 `X-Cluster-Node` 가 실제 실행 노드를 말한다.** 조사할 때 이 값을 먼저 본다 —
같은 요청이 왜 어떤 노드에서는 되고 어떤 노드에서는 안 되는지의 출발점이다.

**배포 시각과 장애 시작 시각을 대조하는 것도 같은 방법이다.** 예: `/structure` 라우팅이
08:16:12 UTC 에 들어갔는데 장애가 07:24:42 UTC 에 시작했다면, 장애 시점의 배포본에는 그
경로가 없었다(`git show f1bc665~1:internal/api/cluster_route.go | grep -c '"/structure"'` → 0).
그래서 **구조 화면만 실패하고 스키마·데이터·SQL 콘솔은 성공**하는 분포가 나온다 — 연결
문제가 아니라 실행 노드가 갈린 것이다.

## 클러스터에서 `Access denied` (Error 1045) — 계정에 노드 주소 추가

**증상**: 같은 DB의 스키마·데이터·SQL 콘솔은 되는데 지표 수집·버전 캡처·백업·복구만
1045로 실패한다. 또는 커넥션 하나가 **모든** 경로에서 실패한다. 노드를 늘린 뒤에 생긴다.

**원인**: MySQL·MariaDB는 사용자를 **이름 + 출발 호스트** 두 개의 조합으로 본다.
...
이름이 같아도 호스트가 다르면 다른 계정이다. 그 주소는 **설정값이 아니라 MySQL이 연결에서
관찰한 출발 주소**라 어느 설정 파일에도 없다. 사설망 VPN(Tailscale 등)이 SNAT하므로
**노드마다 다르다**. 계정이 그중 하나만 덮으면 나머지 노드의 접속이 전부 거절된다 —
비밀번호가 맞아도 거절되므로 오류에 `using password: YES`가 함께 찍힌다.

### 절차 (로컬에서 검증됨)

**1. 어느 주소를 넣어야 하는지 확인한다.** 접속을 한 번 실패시키면 그 값이 문구에 나온다.
...
`실행 노드`가 어느 노드가 시도했는지를, `MySQL이 본 접속 주소`가 **계정에 넣어야 할 값**을
알려 준다. 두 값을 그대로 복사하면 된다.

**2. 현재 계정의 호스트 범위를 본다.** 어느 노드가 빠졌는지 눈으로 확인한다.

```sql
SELECT user, host FROM mysql.user WHERE user = '<PROD_APP_USER>';
```

**3. 빠진 노드의 주소로 계정을 만들고 같은 권한을 준다.**

```sql
CREATE USER IF NOT EXISTS '<PROD_APP_USER>'@'<빠진 노드 IP>' IDENTIFIED BY '<기존과 같은 비밀번호>';
GRANT <기존 계정과 동일한 권한> ON <db>.* TO '<PROD_APP_USER>'@'<빠진 노드 IP>';
FLUSH PRIVILEGES;
```

**4. 결과를 확인한다.** 그 노드에서 실제로 접속해 본다.

```sql
SELECT user, host FROM mysql.user WHERE user = '<PROD_APP_USER>';
SELECT host, user, db FROM information_schema.processlist WHERE user = '<PROD_APP_USER>';
```

### 어느 노드의 주소가 필요한가

| 어떤 기능 | 실행하는 노드 | 담당 노드 주소 | 마스터 주소 |
|---|---|---|---|
| 스키마·데이터·SQL 콘솔·DB 로그·접속 테스트·구조 | 담당 노드 | 필요 | 불필요 |
| 지표 수집·버전 캡처·마이그레이션·드리프트 | 담당 노드가 읽고 **마스터가 기록** | 필요 | **필요** |
| 백업·복구 | 담당 노드 (파일도 마스터로 오간다) | 필요 | **필요** |
| 어시스턴트의 DB 도구 | 담당 노드가 읽고 채팅은 마스터 | 필요 | **필요** |
| 담당 노드가 **없는** DB | 요청을 받은 노드 + 마스터(지표) | — | **필요** |

**마스터 주소를 빠뜨리면** 담당 노드가 지정된 DB의 지표·구조 화면만 실패하고 스키마·데이터는
정상으로 보인다 — 화면마다 성공/실패가 갈려 원인을 찾기 어렵다.

### 확인된 사실

로컬에서 같은 조건(MySQL 하나에 서로 다른 출발 IP로 붙는 컨테이너 둘, 계정을 한쪽 IP로만
제한)을 만들어 검증했다:

```
② 증상 재현   노드 B(계정 있음) → 1
              노드 A(계정 없음) → ERROR 1045 ... 'appuser'@'172.24.0.3' (using password: YES)
③ 조치        CREATE USER 'appuser'@'172.24.0.3' + GRANT
④ 해결 확인   노드 A → 1,  노드 B → 1   ✅
```

운영에서 관찰된 오류 문자열과 같은 모양이고, 위 SQL로 해결된다.

**주의**: 시험할 때 클라이언트를 잘못 고르면 진단이 통째로 틀어진다. alpine의
`mysql-client`는 **MariaDB 판**이라 MySQL 8의 인증 방식(`caching_sha2_password`)을 모른다 —
그것으로 시험하면 계정 문제가 아니라 클라이언트 문제로 실패한다(실제로 이 검증을 처음
돌렸을 때 "절차가 안 통한다"는 잘못된 결론이 나왔다). MySQL 8 서버를 상대할 때는
MySQL 8 클라이언트를 쓴다.
