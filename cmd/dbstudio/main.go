...
func run() error {
	...
	// 하위 명령. 첫 인자가 플래그가 아니면 그것을 명령 이름으로 읽는다.
	//
	// 서버를 띄우는 것이 기본이고, 여기 오는 것들은 **서버를 띄우지 않고**
	// 데이터 디렉터리를 직접 다루는 일이다. 그래서 로그 파일 설정보다 먼저
	// 갈라진다 — 이 명령들은 파일 로그를 열 이유가 없고(사람이 지금 보고 있다),
	// 실행 표식을 남길 이유도 없다(리스닝하지 않으므로 "돌고 있음"이 아니다).
